...
import { 
  ...
  ShieldCheck, AlertCircle, TrendingUp, Settings, Code, Image, ToggleLeft, ToggleRight
} from 'lucide-react';
...
const AdminView: React.FC<AdminViewProps> = ({ onBack }) => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-bgMain flex flex-col items-center justify-center gap-6">
      ...
      <p className="text-[12px] font-black uppercase tracking-[0.5em] text-primary">Establishing Secure Link...</p>
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        {activeTab === 'Users' ? (
            ...
            <div className="bg-surface rounded-[2.5rem] border border-borderMain shadow-2xl overflow-hidden backdrop-blur-3xl">
              <div className="p-8 border-b border-borderMain flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="relative w-full md:w-80">
                  ...
                  <input type="text" placeholder="Filter identities..." className="w-full bg-bgMain rounded-xl pl-12 pr-6 py-2.5 text-xs font-bold border border-borderMain outline-none focus:border-primary/40" onChange={(e) => setSearch(e.target.value)} />
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-borderMain/40 text-textMain">
                    {filteredUsers.map(u => (
                      <tr key={u.id} className="hover:bg-primary/5 transition-all group">
                        <td className="px-10 py-6">
                           <div className="flex items-center gap-4">
                             ...
                             <div className="space-y-0.5">
                                <p className="text-[11px] font-black uppercase">{u.displayName}</p>
                        ...
                        <td className="px-10 py-6 text-xs font-black tabular-nums">{u.credits} units</td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in slide-in-from-bottom-4">
            ...
            <div className="bg-surface p-8 rounded-[2.5rem] border border-borderMain space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                   ...
                   <select 
                      className="w-full bg-bgMain rounded-xl px-4 py-3 text-xs font-bold border border-borderMain outline-none"
            ...
            <button onClick={saveConfig} className="md:col-span-2 w-full py-5 bg-primary text-white font-black uppercase tracking-widest rounded-3xl shadow-xl shadow-primary/20 hover:brightness-110 active:scale-[0.98] transition-all">Synchronize Configuration Clusters</button>
          </div>
        )}
      </div>
    </div>
...
const AdminInput = ({ label, value, onChange }: { label: string, value: string, onChange: (v: string) => void }) => (
  <div className="space-y-1.5">
    <label className="text-[10px] font-black uppercase text-textDim px-1">{label}</label>
