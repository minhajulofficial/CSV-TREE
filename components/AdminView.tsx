...
import { 
  ...
  Zap, Loader2, Trash2, TrendingUp, Code, Image, ToggleLeft, ToggleRight, Save, Layout, Plus, List
} from 'lucide-react';
...
const AdminView: React.FC<AdminViewProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const unsubConfig = onValue(configRef, (snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.val();
        // Support migration from old structure (single ad to list)
        const existingList = data.ads?.list || (data.ads?.image ? [{
          image: data.ads.image,
          link: data.ads.link,
          label: data.ads.label
        }] : DEFAULT_CONFIG.ads.list);
  ...
  const saveConfig = async () => {
    ...
    } catch (err) { alert("Sync Error. Rules violation."); }
  ...
  const addAdUnit = () => {
    setConfig(prev => ({
      ...
      ads: {
        ...prev.ads,
        list: [...prev.ads.list, { image: '', link: '', label: 'SPONSORED' }]
      }
    }));
  };
  ...
  const removeAdUnit = (index: number) => {
    setConfig(prev => ({
      ...
      ads: {
        ...prev.ads,
        list: prev.ads.list.filter((_, i) => i !== index)
      }
    }));
  };
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
            <button onClick={onBack} className="flex items-center gap-2 text-primary text-[10px] font-black uppercase tracking-widest bg-primary/5 px-4 py-2 rounded-full border border-primary/10">
          ...
          <div className="flex bg-surface p-1.5 rounded-2xl border border-borderMain shadow-sm overflow-hidden">
             <button onClick={() => setActiveTab('Users')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'Users' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-textDim'}`}>Users</button>
             <button onClick={() => setActiveTab('System')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'System' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-textDim'}`}>System</button>
          </div>
        </header>
        ...
        {activeTab === 'Users' ? (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              ...
              <StatCard label="Premium Members" value={allUsers.filter(u => u.tier === 'Premium').length} icon={<Shield />} color="text-accent" />
              <StatCard label="Global Credits" value={allUsers.reduce((a, b) => a + (Number(b.credits) || 0), 0)} icon={<CreditCard />} color="text-textMain" />
              <StatCard label="System Status" value="Healthy" icon={<TrendingUp />} color="text-green-500" />
            </div>
            <div className="bg-surface rounded-[2.5rem] border border-borderMain shadow-2xl overflow-hidden backdrop-blur-3xl">
              <div className="p-8 border-b border-borderMain flex flex-col md:flex-row justify-between items-center gap-4">
                <h3 className="text-[12px] font-black uppercase tracking-[0.3em] text-textDim flex items-center gap-3">Operator Registry</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-surface p-8 rounded-[2.5rem] border border-borderMain space-y-6 shadow-sm">
              <div className="flex items-center gap-3 pb-4 border-b border-borderMain"><Code className="text-primary"/><h3 className="text-sm font-black uppercase">Developer Meta</h3></div>
              <div className="space-y-4">
            ...
            <div className="bg-surface p-8 rounded-[2.5rem] border border-borderMain space-y-6 shadow-sm">
              ...
              <div className="space-y-6">
                <div className="space-y-1.5">
                   <label className="text-[10px] font-black uppercase text-textDim px-1">Visibility Level</label>
                   <select className="w-full bg-bgMain rounded-xl px-4 py-3 text-xs font-bold border border-borderMain text-textMain" value={config.ads.visibility} onChange={(e) => setConfig(prev => ({...prev, ads: {...prev.ads, visibility: e.target.value as any}}))}>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between px-1">
                    <span className="text-[10px] font-black uppercase text-textDim tracking-widest flex items-center gap-2"><List size={12}/> Slider Management</span>
                    <button onClick={addAdUnit} className="flex items-center gap-1.5 text-primary text-[10px] font-black uppercase hover:underline"><Plus size={12}/> Add Ad Unit</button>
                  </div>
                  ...
                  <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 no-scrollbar">
                    {config.ads.list.map((ad, idx) => (
                      <div key={idx} className="p-4 bg-bgMain rounded-2xl border border-borderMain space-y-3 relative group/ad">
                        <button onClick={() => removeAdUnit(idx)} className="absolute top-2 right-2 text-red-500/30 hover:text-red-500 transition-colors p-1"><Trash2 size={14}/></button>
                        <div className="text-[8px] font-black text-slate-400 uppercase">Ad Slide #{idx + 1}</div>
                        <div className="grid grid-cols-2 gap-2">
                           <input placeholder="Label (e.g. SPONSORED)" className="text-[10px] bg-white border border-borderMain rounded-lg px-3 py-2 outline-none" value={ad.label} onChange={(e) => updateAdUnit(idx, 'label', e.target.value)} />
                           <input placeholder="Redirect URL" className="text-[10px] bg-white border border-borderMain rounded-lg px-3 py-2 outline-none" value={ad.link} onChange={(e) => updateAdUnit(idx, 'link', e.target.value)} />
                        </div>
                        <input placeholder="Image URL (Landscape optimized)" className="w-full text-[10px] bg-white border border-borderMain rounded-lg px-3 py-2 outline-none" value={ad.image} onChange={(e) => updateAdUnit(idx, 'image', e.target.value)} />
                      </div>
                    ))}
                  </div>
                </div>

                <div className="space-y-1.5 border-t border-borderMain pt-4">
                  <label className="text-[10px] font-black uppercase text-textDim px-1 tracking-widest flex items-center gap-2"><Layout size={12}/> Ad Network Injection</label>
                  <textarea value={config.ads.externalScript || ''} onChange={(e) => setConfig(prev => ({...prev, ads: {...prev.ads, externalScript: e.target.value}}))} className="w-full bg-bgMain rounded-xl px-4 py-3 text-[10px] font-mono border border-borderMain outline-none h-24" placeholder="AdSense / Ezoic code..." />
                </div>
              </div>
            </div>
            <button onClick={saveConfig} disabled={isSaving} className="md:col-span-2 w-full py-5 bg-primary text-white font-black uppercase tracking-widest rounded-3xl shadow-xl shadow-primary/20 hover:brightness-110 flex items-center justify-center gap-3 disabled:opacity-50">
              {isSaving ? <Loader2 className="animate-spin" /> : <Save size={18} />} Synchronize Cluster
            </button>
          </div>
        )}
      </div>
...
const AdminInput = ({ label, value, onChange }: any) => (
  <div className="space-y-1.5">
    ...
    <input type="text" value={value || ''} onChange={(e) => onChange(e.target.value)} className="w-full bg-bgMain rounded-xl px-4 py-3 text-xs font-bold border border-borderMain outline-none focus:border-primary/40 text-textMain" />
  </div>
);
