...
import { 
  ...
  Sun, Moon, Crown, RefreshCw, Sparkles, AlertCircle
} from 'lucide-react';
...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 h-16 bg-bgMain/80 backdrop-blur-2xl border-b border-borderMain flex items-center justify-between px-10 z-50">
        ...
        <div className="flex items-center gap-5">
          <button onClick={toggleTheme} className="p-2.5 rounded-xl bg-bgSidebar border border-borderMain text-textDim hover:text-primary transition-all">
          ...
          {loading ? (
            ...
            <div className="relative" ref={dropdownRef}>
              <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-3 pl-3 pr-1 py-1 rounded-full bg-bgSidebar border border-borderMain hover:border-primary/30 transition-all">
                ...
                <div className="absolute top-full right-0 mt-4 w-[340px] bg-surface border border-borderMain rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-4 z-[100]">
                  <div className="p-8 pb-6 border-b border-borderMain bg-gradient-to-b from-primary/5 to-transparent">
                    <div className="flex items-center gap-4 mb-6">
                      <div className="w-16 h-16 rounded-full bg-primary/10 border-4 border-primary/5 flex items-center justify-center text-primary">
                        {user.photoURL ? <img src={user.photoURL} alt="Avatar" className="w-full h-full rounded-full" /> : <UserIcon size={24} />}
                      </div>
                  ...
                  <div className="p-8 space-y-6">
                    ...
                    <div className="p-4 bg-primary/5 border border-primary/10 rounded-2xl flex items-start gap-3">
                       ...
                       <div className="space-y-1">
                          ...
                          <p className="text-[9px] text-textDim font-bold leading-relaxed italic uppercase">System engines are automatically provisioned. No manual API keys required.</p>
                       </div>
                    </div>
                  </div>
